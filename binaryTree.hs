...
nearestGE EmptyTree x = x-1
nearestGE (Node a left right) x	| (maxValue (Node a left right) < x) = error "value is more than the maximum value in a tree"
								| (a == x) = a
								| (x < a) = let newX = nearestGE left x in
																if (newX < x) then a
																			  else newX
								| (x > a) = nearestGE right x
