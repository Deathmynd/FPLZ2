...
reverseNew :: [a] -> [a]
reverseNew [] = []
reverseNew list = foldlNew (\t h -> h:t) [] list
...
elementAtNew x list | ((length list) <= (fromIntegral x)) = error "no element at list"
    | otherwise = res
      where (res, _) = foldlNew (\(res,x) h -> if (x == 0) then (h, x-1) else (res, x-1)) (head list, x) list
...
indexOfNew x list = if (res == -1) then error "element not found" else res
    where (res, _, _) = foldlNew (\(res, count, x) h -> if (h == x) then (count, count+1, x) else (res, count+1, x)) (-1, 0, x) list
...
test str =
    ...
    let x = elementAtNew 2 [1,2,3,4,5] in
